      ...
      elements.push(
        <div key={`tbl-${i}`} style={{ overflowX: 'auto', margin: '16px 0' }}>
          <table style={dr.table}>
            <thead><tr>{splitRow(rows[0] || '').map((c,ci) => <th key={ci} style={dr.th}>{RI(c.trim(), `th${i}c${ci}`)}</th>)}</tr></thead>
            <tbody>{rows.slice(1).map((row, ri) => (
              <tr key={ri} style={ri % 2 === 1 ? { background: '#f8faff' } : {}}>
                {splitRow(row).map((c, ci) => <td key={ci} style={dr.td}>{RI(c.trim(), `td${i}r${ri}c${ci}`)}</td>)}
              </tr>
            ))}</tbody>
          </table>
        </div>
...
function buildPrintHTML(doc, _unused) {
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith('|')) {
      ...
      body += '<table><thead><tr>' + cells(rows[0]||'').map(c=>`<th>${inlineToHTML(c.trim())}</th>`).join('') + '</tr></thead><tbody>';
      rows.slice(1).forEach((row, ri) => {
        body += `<tr${ri%2===1?' class="alt"':''}>` + cells(row).map(c=>`<td>${inlineToHTML(c.trim())}</td>`).join('') + '</tr>';
      });
      body += '</tbody></table>'; continue;
    }
