...
function Markdown({ text }) {
  ...
  while (i < lines.length) {
    const line = lines[i];
    if (line.startsWith('### '))      { elements.push(<h4 key={i} style={md.h3}>{inline(line.slice(4))}</h4>); }
    else if (line.startsWith('## ')) { elements.push(<h3 key={i} style={md.h2}>{inline(line.slice(3))}</h3>); }
    else if (line.startsWith('# '))  { elements.push(<h2 key={i} style={md.h1}>{inline(line.slice(2))}</h2>); }
    else if (line.startsWith('- ') || line.startsWith('* ')) {
      const items = [];
      while (i < lines.length && (lines[i].startsWith('- ') || lines[i].startsWith('* '))) {
        items.push(<li key={i} style={md.li}>{inline(lines[i].slice(2))}</li>);
        i++;
      }
      ...
      while (i < lines.length && /^\d+\.\s/.test(lines[i])) {
        items.push(<li key={i} style={md.li}>{inline(lines[i].replace(/^\d+\.\s/, ''))}</li>);
        i++;
      }
      ...
      elements.push(<p key={i} style={md.p}>{inline(line)}</p>);
    }
    i++;
  }
...
function inline(text) {
  ...
  while ((m = rx.exec(text)) !== null) {
    ...
    else if (m[4]) parts.push(<code key={m.index} style={md.code}>{m[4]}</code>);
